...
function createGameState() {
    return {
        ...
        turnToKick: null,
        outX: 0, outY: 0, 
        pickupTimeRemaining: 5, kickTimeRemaining: 5,   
        statusMsg: '',
        winner: null,
        lastTick: Date.now()
    };
...
function handleEndLine(state, x, y, defendingTeam) {
    ...
    triggerOut(state, (x < WORLD_W/2 ? 35 : WORLD_W-35), (y < WORLD_H/2 ? 35 : WORLD_H-35), type);
}
...
function goal(state, team) {
    ...
    if (state.score.p1 >= GOAL_LIMIT || state.score.p2 >= GOAL_LIMIT) {
        ...
        setTimeout(() => { initMatch(state); triggerOut(state, 250, 450, "SAQUE", team === 'p1' ? 'p2' : 'p1'); }, 2500);
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', (data) => {
        ...
        if (!rooms[roomCode]) { socket.emit('joinError', 'La sala no existe.'); return; }
        if (Object.values(clients).filter(c => c.room === roomCode).length >= 2) { socket.emit('joinError', 'Sala llena.'); return; }
    ...
    socket.on('playerAction', (action) => {
        ...
        if (action.type === 'move') player.target = action.target;
        else if (action.type === 'shoot' && state.ballOwnerId === player.id) {
            if (state.isOut && state.turnToKick !== player.team) return;
            state.ballOwnerId = null; state.ball.vx = action.vx; state.ball.vy = action.vy; state.ball.lastTeam = player.team;
            player.immune = 20; if (state.isOut) { state.isOut = false; state.statusMsg = ''; }
        }
    });
    ...
    function handlePlayerLeave(socketId) {
        ...
        if (client) {
            const roomCode = client.room; delete clients[socketId];
            if (Object.values(clients).filter(c => c.room === roomCode).length === 0) delete rooms[roomCode];
            else if (rooms[roomCode]) {
                rooms[roomCode].playersReady = { p1: false, p2: false };
                rooms[roomCode].status = 'waiting';
                io.to(roomCode).emit('stateUpdate', rooms[roomCode]);
            }
        }
    }
    socket.on('leaveRoom', () => { if (clients[socket.id]) { socket.leave(clients[socket.id].room); handlePlayerLeave(socket.id); } });
...
setInterval(() => {
    for (const [roomCode, state] of Object.entries(rooms)) {
        ...
        if (!state.isGoal && !state.isOut) {
            if (now - state.lastTick >= 1000) { state.timeRemaining--; state.lastTick = now; if (state.timeRemaining <= 0) state.status = 'ended'; }
        } else state.lastTick = now;
        
        state.players.forEach(p => {
            if (p.immune > 0) p.immune--;
            if (state.isOut && p.team !== state.turnToKick && Math.hypot(p.x - state.outX, p.y - state.outY) < 85) {
                const angle = Math.atan2(p.y - state.outY, p.x - state.outX);
                p.x = state.outX + Math.cos(angle) * 85; p.y = state.outY + Math.sin(angle) * 85; p.target = null;
            }
            if (p.target) {
                const dx = p.target.x - p.x, dy = p.target.y - p.y, d = Math.hypot(dx, dy);
                if (d > 5) { p.x += (dx/d) * CONFIG.PLAYER_SPEED; p.y += (dy/d) * CONFIG.PLAYER_SPEED; } else p.target = null;
            }
        ...
        if (state.isOut || state.isGoal) {
            ...
            if (!ballOwner && state.isOut) {
                ...
                if (state.pickupTimeRemaining <= 0) { state.turnToKick = state.turnToKick === 'p1' ? 'p2' : 'p1'; state.pickupTimeRemaining = 5; state.statusMsg = `FALTA TIEMPO`; }
                else state.players.forEach(p => { if (p.team === state.turnToKick && Math.hypot(p.x - state.ball.x, p.y - state.ball.y) < 45) { state.ballOwnerId = p.id; state.kickTimeRemaining = 5; p.x = state.outX; p.y = state.outY; p.target = null; } });
            }
            if (ballOwner = state.players.find(p => p.id === state.ballOwnerId)) {
                if (state.isOut) { state.ball.x = state.outX; state.ball.y = state.outY; state.kickTimeRemaining -= 1/60; 
                    if (state.kickTimeRemaining <= 0) { state.turnToKick = state.turnToKick === 'p1' ? 'p2' : 'p1'; state.ballOwnerId = null; state.statusMsg = `FALTA TIEMPO`; ballOwner.immune = 120; }
                } else { state.ball.x = ballOwner.x; state.ball.y = ballOwner.y; }
            }
        } else {
            let ballOwner = state.players.find(p => p.id === state.ballOwnerId);
            if (ballOwner) { state.ball.x = ballOwner.x; state.ball.y = ballOwner.y; state.ball.vx = 0; state.ball.vy = 0; 
                state.players.forEach(p => { if (p.team !== ballOwner.team && p.immune === 0 && ballOwner.immune === 0 && Math.hypot(p.x - state.ball.x, p.y - state.ball.y) < CONFIG.STEAL_DIST && Math.random() < CONFIG.STEAL_PROB) { state.ballOwnerId = p.id; p.immune = CONFIG.IMMUNE_TIME; } });
            } else {
                state.ball.x += state.ball.vx; state.ball.y += state.ball.vy; state.ball.vx *= 0.99; state.ball.vy *= 0.99;
                if (state.ball.y < 15) { if (state.ball.x > CONFIG.POST_L && state.ball.x < CONFIG.POST_R) goal(state, 'p1'); else handleEndLine(state, state.ball.x, 35, 'p2'); }
                else if (state.ball.y > WORLD_H - 15) { if (state.ball.x > CONFIG.POST_L && state.ball.x < CONFIG.POST_R) goal(state, 'p2'); else handleEndLine(state, state.ball.x, WORLD_H - 35, 'p1'); }
                else if (state.ball.x < 15) triggerOut(state, 35, state.ball.y, "BANDA");
                else if (state.ball.x > WORLD_W - 15) triggerOut(state, WORLD_W - 35, state.ball.y, "BANDA");
                state.players.forEach(p => { if (Math.hypot(state.ball.x - p.x, state.ball.y - p.y) < CONFIG.PLAYER_RAD + CONFIG.BALL_RAD && p.immune === 0) state.ballOwnerId = p.id; });
            }
        }
...
server.listen(PORT, '0.0.0.0', () => { console.log(`Servidor de Futsal corriendo en el puerto ${PORT}`); });
